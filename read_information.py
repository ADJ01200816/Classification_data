...
def get_level():
    ...
    int_level = []
    for str in level:
        str = str.strip()  # Remove whitespace characters from both ends of the string
        try:
            int_level.append(int(str))  # Trying to convert a string to an integer
        except ValueError:
...
def get_question(question_range = []):
    ...
    question = []
    for i in range(num):
        question.append("Please extract the following named entities from Material C："+attribute[i])
    if question_range != []:
        narrow_question = []
